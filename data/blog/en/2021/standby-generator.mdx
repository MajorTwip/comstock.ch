...
authors: ["MajorTwip"]
language: en
images: ['/static/images/blog/standby-generator/Screenshot_20210322-171823_Piktures2.jpg']
summary: "As part of the power supply concept for servers etc., I extended a generator with electric start. Originally it's an ordinary generator with a manual cold start lever and a pull cord. Additionally, it now has a key switch to operate the electric starter. The cold start remains manual. An Arduino now handles the entire starting process."
---

As part of the power supply concept for servers etc., I extended a generator with electric start. Originally it's an ordinary generator with a manual cold start lever and a pull cord. Additionally, it now has a key switch to operate the electric starter. The cold start remains manual.

An Arduino now handles the entire starting process.

The hardware setup looks something like this:  

- Arduino Micro (clone)
- Serial connection to a Raspberry Pi which will send the start command
- Relay that operates the electric starter motor
- Relay that connects the spark plug to ground to shut off the engine
- DC from the generator (before the diode) via voltage divider/Zener to the Arduino's ADC
- Servo that operates the cold start (now replaced by an electromagnet)

The Arduino handles the following functions:

1. Regular reporting of the battery status (from the ADC) via Serial
2. Waiting for commands
3. Upon command, pulling the cold start
4. Releasing the shutdown relay
5. Activating the starter relay for a certain duration
6. Waiting, then measuring the RPM (via ADC, which now shows battery voltage plus rhythmic diode voltage)
7. If the engine is not running, a new startup process, now 5 seconds longer
8. If the engine is running, releasing the cold start
9. Regular reporting and monitoring

All timings are configurable

Code [here](https://raw.githubusercontent.com/MajorTwip/PowerGovernor/master/GenertorProMicro.ino), this is old... I will show a newer version soon

https://youtu.be/HBANZNmgIr8
